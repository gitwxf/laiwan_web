...
namespace LaiWanPayMvc.Business
{
    public class JsApiPay
    {
        ...
        public JsApiPay(HttpContext context)
        {
        ...
        */
        ...
        {
            if (!string.IsNullOrEmpty(currentContext.Request.QueryString["code"]))
            {
                //获取code码，以获取openid和access_token
                string code = currentContext.Request.QueryString["code"];
            ...
            {
                //构造网页授权获取code的URL
                string host = currentContext.Request.Url.Host;
                string path = currentContext.Request.Path;
                string redirect_uri = HttpUtility.UrlEncode("http://" + host + path);
                LogUtils.WriteLogFile(redirect_uri, "redirect_uri");
                ...
                {
                    //触发微信返回code码         
                    currentContext.Response.Redirect(url);//Redirect函数会抛出ThreadAbortException异常，不用处理这个异常
                }
	    ...
	    */
        ...
        /// 调用统一下单，获得下单结果
        /// </summary>
        /// <param name="strDesc">body描述</param>
        ...
        public WxPayData GetUnifiedOrderResult(string strDesc,string strattach,string strOrderId,string strTag,string notifyUrl)
        {
            //统一下单
            WxPayData data = new WxPayData();
            data.SetValue("body", strDesc);
            data.SetValue("attach", strattach);
            data.SetValue("out_trade_no", strOrderId);
	    ...
	    */
        public string GetEditAddressParameters()
	    {
            ...
            try
            {
                string host = currentContext.Request.Url.Host;
                string path = currentContext.Request.Path;
                string queryString = currentContext.Request.Url.Query;
