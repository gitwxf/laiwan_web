...
{
    public partial class AgentsBindAccounts : System.Web.UI.Page
    {
        ...
        protected void Bind_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(strOpenId))
            {
                ...
                if (result == 1)
                {
                    ...
                    {
                        this.ClientScript.RegisterStartupScript(this.GetType(), "key", "<script type='text/javascript'>alert('绑定成功！');location.href='MobilePayDefault.aspx?AgentsID=" + dt.Rows[0]["AgentsID"] + "&KindID=" + dt.Rows[0]["KindID"] + "';</script>");
                        return;
                    }
                    else
                    {
